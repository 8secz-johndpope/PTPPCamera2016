...
- (void)assetDeselected:(ELCAsset *)asset
{
    ...
    long numOfSelectedElements = [[ELCConsole mainConsole] numOfSelectedElements];
    if (asset.index < numOfSelectedElements - 1) {
        ...
        for (int i = 0; i < [self.elcAssets count]; i++) {
            ELCAsset *assetInArray = [self.elcAssets safeObjectAtIndex:i];
